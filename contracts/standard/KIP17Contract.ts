import { BigNumberish, ContractInterface, getBigInt } from "ethers";
...
export default class KIP17Contract extends Contract {
    ...
    public async balanceOf(owner: string): Promise<BigInt> {
        return BigInt.from(await this.runMethod("balanceOf", owner));
    }
    ...
    public async tokenOfOwnerByIndex(owner: string, index: BigNumberish): Promise<BigInt> {
        return BigInt.from(await this.runMethod("tokenOfOwnerByIndex", owner, index));
    }
}
